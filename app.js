    ...
    if (cfg.type === "diagram") {
      ...
      const datasets = cfg.datasett.map(function(ds, i) {
        return {
          ...
          fill:            isLinje,
          tension:         isLinje ? 0.3 : 0,
          pointBackgroundColor: G.c800,
          pointRadius:     isLinje ? 4 : 0
        };
      });
      new Chart(canvas, {
  ...
  function renderInnhold(innhold, planId) {
    ...
    innhold
      ...
      .forEach(function(rad) {
        ...
        // Normalisér feltnavnkonvensjoner
        const brodtekst = rad.brodtekst     || rad.innhold          || null;
        const mediaUrl  = rad.mediaUrl      || rad.bildeUrl         || null;
        const pbiUrl    = rad.pbiUrl        || null;
        const jsonData  = rad.json          || rad.innhold_url_json  || null;
        ...
        // Overskrift
        if (rad.overskrift) {
          const h3 = document.createElement("h3"); h3.textContent = rad.overskrift; blokk.appendChild(h3);
        }
        ...
        // JSON (diagram, nøkkeltall, tabell)
        if (jsonData) {
